...
export STEAMP_WORKSPACE="default"
...
# For example: `aws_securityhub_master`
export STEAMP_AWS_CON_SH_MASTER="aws"
...
# For example: `aws_all`
export STEAMP_AWS_CON_AGGREGATOR="aws"
# Set the name of the local table
export LOCAL_TABLE="local_aws_tagging_resource"

echo "Creating Table: $LOCAL_TABLE"
steampipe --workspace $STEAMP_WORKSPACE query "DROP TABLE IF EXISTS $LOCAL_TABLE;" && \
steampipe --workspace $STEAMP_WORKSPACE query "CREATE TABLE $LOCAL_TABLE AS select * FROM $STEAMP_AWS_CON_AGGREGATOR.$TABLE;" && \
echo "Ok" || echo "Error"
